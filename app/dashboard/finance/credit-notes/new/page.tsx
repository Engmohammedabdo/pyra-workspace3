...
export default function NewCreditNotePage() {
  const t = useTranslations('finance.creditNotes.new');
  ...
  useEffect(() => {
    ...
    fetch(`/api/invoices/${invoiceId}`)
      ...
      .then(json => {
        ...
        // i18n-exempt: stored data — this auto-fill becomes the persisted
  ...
  const createMutation = useMutation({
    mutationFn: (data: object) => mutateAPI<{ id?: string }>('/api/dashboard/credit-notes', 'POST', data),
    onSuccess: (data) => {
      toast.success(t('toasts.createSuccess'));
      router.push(`/dashboard/finance/credit-notes/${(data as any).id}`);
    },
    onError: () => toast.error(t('toasts.createFailed')),
  });

  const handleSubmit = () => {
    if (!reason.trim()) { toast.error(t('toasts.reasonRequired')); return; }
    const validItems = items.filter(item => item.description.trim());
    if (validItems.length === 0) { toast.error(t('toasts.itemRequired')); return; }
    createMutation.mutate({
      invoice_id: linkedInvoiceId || null, client_id: clientId || null,
      reason, issue_date: issueDate, notes: notes || null, vat_rate: vatRate,
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select value={clientId} onValueChange={setClientId}>
                ...
                <SelectContent>
                  {(clients as any[]).map((c: any) => <SelectItem key={c.id} value={c.id}>{c.name}{c.company ? ` — ${c.company}` : ''}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Input value={linkedInvoiceId} onChange={e => setLinkedInvoiceId(e.target.value)} placeholder={t('fields.linkedInvoicePlaceholder')} dir="ltr" />
          </div>
          <div className="space-y-2">
            <Label>{t('fields.reason')} <span className="text-destructive">*</span></Label>
            <Textarea value={reason} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setReason(e.target.value)} placeholder={t('fields.reasonPlaceholder')} rows={2} />
          </div>
          <div className="space-y-2">
      ...
      <div className="flex items-center gap-3 justify-end">
        <Link href="/dashboard/finance/credit-notes"><Button variant="outline">{t('cancel')}</Button></Link>
