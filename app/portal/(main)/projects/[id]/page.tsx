    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="progress" className="mt-4">
          <ProjectPipelineProgress projectId={project.id} />
        </TabsContent>
...
/* ── Project Pipeline Progress (Portal) ── */
...
function ProjectPipelineProgress({ projectId }: { projectId: string }) {
  ...
  if (!data?.has_pipeline || data.boards.length === 0) {
    return (
      <EmptyState
        icon={GitBranch}
        title="لا توجد مراحل متابعة"
        description="لم يتم إعداد خط إنتاج لهذا المشروع بعد"
      />
    );
  }
        ...
        <Card key={board.board_id}>
          <CardHeader className="pb-3">
            ...
            <div className="w-full h-3 bg-muted rounded-full overflow-hidden mt-2">
              <div
                className="h-full bg-gradient-to-l from-emerald-500 to-emerald-400 rounded-full transition-all duration-700"
                style={{ width: `${board.progress_percent}%` }}
              />
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {board.completed_tasks} مهمة مكتملة من {board.total_tasks}
            </p>
          </CardHeader>
          <CardContent className="pt-0">
            {/* Stages as horizontal steps */}
            <div className="flex items-center gap-2 overflow-x-auto pb-1" dir="ltr">
              {board.stages.map((stage, idx) => {
                ...
                const isLast = idx === board.stages.length - 1;
                return (
                  <div key={idx} className="flex items-center">
                    <div className={`px-3 py-2 rounded-lg ${colors.bg} min-w-[100px] text-center ${
                      stage.is_current ? 'ring-2 ring-emerald-500/50' : ''
                    } ${stage.is_done ? 'opacity-60' : ''}`}>
                      <div className="flex items-center justify-center gap-1.5 mb-0.5">
                        <div className={`w-2 h-2 rounded-full ${colors.dot}`} />
                        <span className="text-xs font-medium" dir="rtl">{stage.name}</span>
                      </div>
                      <span className="text-[10px] text-muted-foreground">
                        {stage.task_count} {stage.task_count === 1 ? 'مهمة' : 'مهام'}
                      </span>
