...
export async function GET(
  ...
  try {
    ...
    const [leadRes, activitiesRes, contractsRes] = await Promise.all([
      supabase
    ...
    const [clientRes, invoicesRes, milestonesRes] = await Promise.all([
      ...
      contractIds.length
        ? supabase
            .from('pyra_invoices')
            .select('id, contract_id, invoice_number, status, issue_date, due_date, subtotal, total, amount_paid')
    ...
    // Top-level customer KPIs.
    const ltv = enrichedContracts.reduce((acc, c) => acc + c.kpis.total_paid, 0);
    const mrr = enrichedContracts
      .filter((c) => c.type === 'retainer' && c.status === 'active')
      .reduce((acc, c) => {
        const months = cycleMonths[(c.retainer_cycle || 'monthly').toLowerCase()] ?? 1;
        return acc + c.retainer_amount / months;
      }, 0);
    ...
    // Recency (30%): days since most recent activity
    let recencyScore = 0;
    let daysSinceLastActivity: number | null = null;
    if (activities.length > 0) {
      const mostRecent = new Date(activities[0].created_at).getTime();
      ...
      // Fall back to lead.last_contact_at if no activities row but contact tracked.
      const days = Math.floor((Date.now() - new Date(lead.last_contact_at).getTime()) / (24 * 60 * 60 * 1000));
      daysSinceLastActivity = days;
      recencyScore = days < 7 ? 30 : days < 30 ? 20 : days < 90 ? 10 : 0;
    }

    // Payment (30%): % of last-180d invoices paid on or before due_date
    const recentInvoices = enrichedContracts
