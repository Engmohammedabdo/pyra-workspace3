...
# القواعد التقنية / Technical rules — **مش هنا**

البريفنج فوق هو الكونتكست الأولاني: مين أبو، وإزاي تشتغل معاه. القواعد التقنية
**كلها** في مكان واحد، عشان ماتتنسخش وتفترق:

| اقرا ده | لإيه |
|---|---|
| **[`CLAUDE.md`](CLAUDE.md)** | **المرجع الوحيد للقواعد التشغيلية** — الأوامر، طبقة الداتا (React Query)، الصلاحيات، RTL/dark-mode، الميجريشنز، الديبلوي |
| [`docs/decisions/`](docs/decisions/) | أرشيف القرارات المقفولة (CRM / finance / HR / security / platform). فهرسها في آخر `CLAUDE.md` |
| [`DATABASE-SCHEMA.md`](DATABASE-SCHEMA.md) · [`docs/SYSTEM-STRUCTURE.md`](docs/SYSTEM-STRUCTURE.md) | السكيما الكاملة + المرجع الشامل |

> **ليه مفيش نسخة من القواعد هنا؟** كان فيه — `AGENTS.md` كان نسخة 4,700 سطر من
> `CLAUDE.md`، وبدأت تفترق فعلاً (كانت بتقول i18n «Phases 0–4» والصح «0–5 + 6a/6b»،
> وناقصها 4 أقسام). نسختين × 250 KB بتتصانوا يدوي = افتراق مضمون. اتشالت 2026-07-26.

**قبل أول سطر كود:** اقرا `CLAUDE.md` كامل — هو ~830 سطر بس دلوقتي — وشوف فهرس
القرارات المقفولة في آخره عشان تعرف إيه المتقفل في المنطقة اللي هتلمسها.
