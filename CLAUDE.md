...
lib/mobile/parse-calls.ts    → parseCallsBatch() — pure calls/sync parsing. A bad ROW is dropped (and logged); only the ENVELOPE 422s. NEVER report a dropped row as status 'error' — that freezes the device cursor forever
lib/calls/colleague-call-notice.ts → buildColleagueCallNotice() — F-07 owner notice. Self-contained on purpose: the ownership gate writes nothing to the lead, so this is the owner's ONLY record
pyra-calls-app/…/core/AppGate.kt → decide(granted, loggedIn, sessionDead, blocked) — the app's top-level branch ORDER, pure + 16-case tested. SESSION_DEAD_BLOCKED is the INTERSECTION only
...
| [Calls: the trapped phone, the frozen sync, the owner's notice](docs/decisions/crm.md#calls-the-trapped-phone-the-frozen-sync-and-the-owners-notice-locked-2026-08-10) | Wave د closure (B-14/B-15/F-07/T-01–T-05). **Never show a blocking screen to someone who cannot complete its action** — a revoked key is that condition in disguise, and both escape hatches sat behind the dead key; a bad call row is DROPPED and `status:'error'` is forbidden (it freezes the cursor); the device gate asks for **`leads.view`, never a role** (`extra_permissions` grant without the role) and must be checked against prod before shipping; F-07's notice is verbose because the lead timeline is empty **by design**; `@Synchronized` on `AppPrefs` guards nothing (seven construction sites); v1.7.1 is non-mandatory **because B-15 fixes the mandatory trap itself** |
| [Calls: the cron that wrote its own sort key](docs/decisions/crm.md#calls-wave-د-1-the-cron-that-wrote-its-own-sort-key-locked-2026-08-12) | Wave د+ #1 (4 productivity features, v1.8.0). **A cron must never rank on a value it writes itself** — the idle nudge sorted on a timestamp its own `idle_warning` row reset, so a nudged lead re-won the cap for ever and ~85% of the pool would never be nudged again (measured: the cron's own row is the newest activity on 619 of 772 eligible leads); recency belongs to ELIGIBILITY, ordering is rotation. **The server may never reject what the live app cannot send** — enforcement is gated on `x-app-version`, fails OPEN, parses strictly (`parseInt` accepts `'11abc'`), and must be proven BOTH directions on prod before the APK ships. A blocked control always explains itself from ONE guard; two tabs about the same leads share ONE stage list; a scheduling module's array lengths are guarded against the live constant so it can never emit `NaN`. And: **7 of 9 defects came from the plan's own example code** — read the real file before applying a snippet |
