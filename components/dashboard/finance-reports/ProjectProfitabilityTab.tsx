...
export function ProjectProfitabilityTab() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetchAPI<{ projects?: ProjectProfitability[] }>(`/api/finance/reports/project-profitability?from=${from}&to=${to}`)
      ...
      .catch(() => { toast.error(t('projectProfitability.loadFailed')); setData([]); })
  ...
  return (
    <div className="space-y-6">
      ...
      {!data || data.length === 0 ? <EmptyState icon={FileText} title={t('projectProfitability.noDataToDisplay')} /> : (
        <Card>
          <CardHeader><CardTitle className="text-base">{t('projectProfitability.title')}</CardTitle></CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b text-muted-foreground"><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.project')}</th><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.budget')}</th><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.revenue')}</th><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.expenses')}</th><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.profit')}</th><th className="text-end py-3 px-2 font-medium">{t('projectProfitability.table.budgetUtilization')}</th></tr></thead>
