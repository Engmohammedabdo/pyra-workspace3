...
export function ProjectProfitabilityTab() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(`/api/finance/reports/project-profitability?from=${from}&to=${to}`)
      .then((r) => r.json())
      .then((res) => { if (res.data?.projects) setData(res.data.projects); else setData([]); })
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b text-muted-foreground"><th className="text-right py-3 px-2 font-medium">المشروع</th><th className="text-right py-3 px-2 font-medium">الميزانية</th><th className="text-right py-3 px-2 font-medium">الإيرادات</th><th className="text-right py-3 px-2 font-medium">المصاريف</th><th className="text-right py-3 px-2 font-medium">الربح</th><th className="text-right py-3 px-2 font-medium">استخدام الميزانية</th></tr></thead>
