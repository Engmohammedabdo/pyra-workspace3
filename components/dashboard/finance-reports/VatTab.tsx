...
export function VatTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!data ? <EmptyState icon={FileText} title="لا توجد بيانات ضريبية" /> : (
        <>
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
            <SummaryCard title="ضريبة محصّلة" value={formatCurrency(data.summary.vat_collected)} icon={TrendingUp} colorClass="text-green-600 dark:text-green-400" />
            <SummaryCard title="ضريبة مدفوعة" value={formatCurrency(data.summary.vat_paid)} icon={TrendingDown} colorClass="text-red-600 dark:text-red-400" />
            <SummaryCard title="صافي الضريبة" value={formatCurrency(data.summary.net_vat)} icon={Receipt} colorClass={data.summary.net_vat >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'} />
          </div>
          <Card>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tfoot><tr className="border-t-2 font-bold"><td className="py-3 px-2">الإجمالي</td><td className="py-3 px-2 font-mono text-green-600 dark:text-green-400">{formatCurrency(data.summary.vat_collected)}</td><td className="py-3 px-2 font-mono text-red-600 dark:text-red-400">{formatCurrency(data.summary.vat_paid)}</td><td className={`py-3 px-2 font-mono ${data.summary.net_vat >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{formatCurrency(data.summary.net_vat)}</td></tr></tfoot>
                </table>
              </div>
            </CardContent>
          </Card>
