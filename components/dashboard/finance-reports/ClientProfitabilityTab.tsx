...
export function ClientProfitabilityTab() {
  ...
  const fetchData = useCallback(() => {
    ...
    fetchAPI<ClientProfitability[]>(`/api/finance/reports/client-profitability?from=${from}&to=${to}`)
      ...
      .catch(() => toast.error(t('clientProfitability.loadFailed')))
  ...
  return (
    <div className="space-y-6">
      ...
      {!data || data.length === 0 ? <EmptyState icon={FileText} title={t('clientProfitability.noDataToDisplay')} /> : (
        <Card>
          <CardHeader><CardTitle className="text-base">{t('clientProfitability.title')}</CardTitle></CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b text-muted-foreground"><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.client')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.company')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.revenue')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.expenses')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.profit')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.margin')}</th><th className="text-end py-3 px-2 font-medium">{t('clientProfitability.table.contracts')}</th></tr></thead>
