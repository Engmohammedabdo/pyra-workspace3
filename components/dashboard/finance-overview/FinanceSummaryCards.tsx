    ...
    <StaggerContainer className="grid gap-4 grid-cols-1 md:grid-cols-3">
        ...
        <Card className="transition-all duration-200 hover:shadow-md hover:border-green-500/30">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-muted-foreground">{t('moneyIn')}</span>
            ...
            <p className="text-xs text-muted-foreground mt-2">{t('ytdLabel', { amount: formatCurrency(summary.revenue_ytd) })}</p>
          </CardContent>
        </Card>
      </StaggerItem>
      ...
      <StaggerItem>
        <Card className="transition-all duration-200 hover:shadow-md hover:border-red-500/30">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-muted-foreground">{t('moneyOut')}</span>
            ...
            <p className="text-xs text-muted-foreground mt-2">{t('ytdLabel', { amount: formatCurrency(summary.expenses_ytd) })}</p>
          </CardContent>
        </Card>
      </StaggerItem>
      ...
      <StaggerItem>
        <Card className={`transition-all duration-200 hover:shadow-md ${summary.profit_mtd >= 0 ? 'hover:border-green-500/30' : 'hover:border-red-500/30'}`}>
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-muted-foreground">{t('net')}</span>
            ...
            <div className="flex items-baseline gap-2">
              ...
              {summary.revenue_mtd > 0 && <Badge variant={summary.profit_mtd >= 0 ? 'default' : 'destructive'} className="text-xs">{t('marginBadge', { percent: getProfitMargin(summary.revenue_mtd, summary.expenses_mtd) })}</Badge>}
            </div>
            <p className="text-xs text-muted-foreground mt-2">{t('ytdLabel', { amount: formatCurrency(summary.profit_ytd) })}</p>
          </CardContent>
        </Card>
      </StaggerItem>
    </StaggerContainer>
