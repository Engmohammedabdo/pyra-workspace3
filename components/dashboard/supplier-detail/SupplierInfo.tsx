...
export function SupplierInfo({ supplier }: { supplier: any }) {
  const t = useTranslations('finance.suppliers.detail.info');
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card><CardContent className="p-4 space-y-2">
          <p className="text-sm text-muted-foreground">{t('contactTitle')}</p>
        ...
        <Card><CardContent className="p-4 space-y-2">
          <p className="text-sm text-muted-foreground">{t('financialTitle')}</p>
          <p className="text-sm flex items-center gap-2"><Building2 className="h-4 w-4 text-muted-foreground" /> {t('taxNumber', { value: supplier.tax_number || '—' })}</p>
          <p className="text-sm">{t('paymentTerms', { days: supplier.payment_terms_days })}</p>
          <p className="text-sm">{t('currency', { value: supplier.currency })}</p>
        </CardContent></Card>
      </div>
      ...
      {(supplier.bank_name || supplier.bank_account || supplier.bank_iban) && (
        <Card>
          <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm p-4">
            <div><p className="text-muted-foreground">{t('bank')}</p><p className="font-medium">{supplier.bank_name || '—'}</p></div>
            <div><p className="text-muted-foreground">{t('bankAccount')}</p><p className="font-medium font-mono" dir="ltr">{supplier.bank_account || '—'}</p></div>
            <div><p className="text-muted-foreground">{t('iban')}</p><p className="font-medium font-mono" dir="ltr">{supplier.bank_iban || '—'}</p></div>
          </CardContent>
        </Card>
      )}
