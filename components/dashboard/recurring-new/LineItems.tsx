...
export function LineItems({ items, currency, updateItem, addItem, removeItem }: LineItemsProps) {
  const t = useTranslations('finance.recurring.new.lineItems');
        ...
        {items.map((item, index) => (
          <div key={index} className="grid grid-cols-1 md:grid-cols-[1fr_120px_120px_120px_40px] gap-3 items-start border-b pb-4 last:border-0 last:pb-0">
            <div className="space-y-1">
              <Label className="md:hidden text-xs text-muted-foreground">{t('descriptionLabel')}</Label>
              <Input value={item.description} onChange={(e) => updateItem(index, 'description', e.target.value)} placeholder={t('descriptionPlaceholder')} />
            </div>
            <div className="space-y-1">
              <Label className="md:hidden text-xs text-muted-foreground">{t('quantityLabel')}</Label>
            ...
            <div className="space-y-1">
              <Label className="md:hidden text-xs text-muted-foreground">{t('rateLabel')}</Label>
            ...
            <div className="space-y-1">
              <Label className="md:hidden text-xs text-muted-foreground">{t('amountLabel')}</Label>
              <Input type="number" value={item.amount.toFixed(2)} readOnly className="bg-muted" dir="ltr" />
            </div>
            <div className="flex items-end">
              <Button type="button" variant="ghost" size="icon" className="text-destructive" onClick={() => removeItem(index)} disabled={items.length <= 1} aria-label={t('removeItem')}>
