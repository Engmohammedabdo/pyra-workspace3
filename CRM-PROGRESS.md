...
Scope discipline observed: each commit shipped with standard orchestra cadence (Investigator → Implementer → Reviewer → user adjudication → post-deploy verify). Reviewer caught 1 false-positive + 4 real bugs across the phase (Commit 2 H1 dual-bucket completed lead task; Commit 4 CRITICAL TZ slippage in follow-up + meeting start fields; Commit 5 HIGH 1 Dubai TZ today-key + HIGH 2 RTL chevron convention).

### Sub-step commits

| # | Commit | What landed |
|---|---|---|
| 1 | `ce2aa57` | **@-mentions in CRM lead activities + highlight UX.** Extended existing `MentionTextarea` (Phase 6) with `forwardRef` + `leadId` prop priority over `taskId`/`projectId`. NEW `/api/dashboard/leads/[id]/members` endpoint (admin + sales_agent pool sorted by display_name). Activity-composer JSX swap from raw `<Textarea>`. POST `/api/crm/leads/[id]/activities` extended with fire-and-forget `notifyMany()` IIFE — resolves Arabic-aware `@token` against username + display_name lookup, dedupes via Set, excludes self, single notify call. Deep-link `?tab=activity&highlight={la_id}` opens lead detail on Activity tab; `ActivityTimeline` useEffect scrolls + flashes ring-2 for 2s with full cleanup (cancelAnimationFrame + clearTimeout + classList.remove on unmount). `data-activity-id` data-attribute pattern on each `<ActivityItem>` enables `querySelector(CSS.escape(id))` lookup. 8 files (1 NEW + 7 MOD), +277/-17 LOC. Reviewer LIGHT PASSED 6/6. |
| 2 | `cd07907` | **Lead-attached tasks (CRUD + my-tasks union).** Migration 018 — `pyra_lead_tasks` (12 cols, 2 indexes, 3 CHECK constraints) — independent table NOT a reuse of `pyra_tasks` (lead lifecycle doesn't live on project boards). GET/POST `/api/crm/leads/[id]/tasks` + PATCH/DELETE `/api/crm/leads/[id]/tasks/[taskId]` with `leads.update` + `canAccessLead` + cross-resource guard (`WHERE id=taskId AND lead_id=leadId` double-eq per Phase 15.2 attachments precedent). DELETE permission: admin OR creator. `useLeadTasks` / `useCreateLeadTask` / `useUpdateLeadTask` / `useDeleteLeadTask` React Query hooks. `app/api/my-tasks/route.ts` extended to union board + lead tasks with `_source` discriminator + `target_path` (FIX 1: `?tab=tasks` future-correct path). 4 user-approved fixes: completed_at lifecycle (FIX 2 — NOW on transition TO 'completed', NULL on transition AWAY), CASE-based status sort (FIX 3 — pending<in_progress<completed<cancelled), title validation BEFORE DB CHECK with Arabic message (FIX 4). 7 files (4 NEW + 3 MOD), +1127/-23 LOC. Smoke tests 37/37. Reviewer MEDIUM CONDITIONAL PASS — fixed H1 (synthetic due_date fallback to completed_at for done-bucket routing), M1 (self-contained JS sort with explicit tiebreakers — no V8 stability dependency), L1 (extracted `LEAD_TASK_TITLE_MAX` to `lib/constants/statuses.ts`). |
| 3 | `b68acbe` | **Lead tasks UI tab + my-tasks source-aware rendering.** NEW `<LeadTasksTab>` — add form (collapsed by default with `+ إضافة مهمة` h-11 trigger, expands inline), list, Edit Sheet (full field edit via kebab), Delete AlertDialog (NOT `window.confirm` — Phase 15.2 standard). NEW `<LeadTaskRow>` — inline-edit status checkbox + click-to-edit title (Enter saves, Escape cancels); kebab dropdown with تعديل التفاصيل + حذف. Priority chips with icons (AlertOctagon/AlertTriangle/Minus/ArrowDown). Completed task styling: line-through title + opacity-60 + bg-muted/30. `formatTaskDueDate` helper added to `lib/utils/format.ts` — 5 friendly buckets (متأخر/اليوم/غداً/بعد N أيام/absolute date) via date-fns `differenceInCalendarDays`. my-tasks-client.tsx: source-aware Link via `task.target_path` + StickyNote icon + "عميل محتمل" sub-label for lead tasks (vs Circle + project/board path for board tasks). 6 files (2 NEW + 4 MOD), +898/-15 LOC. Reviewer LIGHT CONDITIONAL PASS — i18n fix applied ("Lead" → "عميل محتمل"); Reviewer's HIGH claim was a false positive (lead tasks DO bucket into "done" via the synthetic `pyra_board_columns.is_done_column = true` stub in my-tasks API projection — Reviewer missed `app/api/my-tasks/route.ts:119-124`). |
| 4 | `798ac3a` | **Calendar events unified feed API.** Read-only GET `/api/calendar/events` aggregates 3 sources: `pyra_lead_tasks` (due_date, status != cancelled), `pyra_sales_follow_ups` (due_at, status != cancelled), `pyra_lead_activities` (activity_type='meeting_scheduled', metadata.meeting_date IS NOT NULL). Pre-flight grep verified: `metadata.meeting_date` is ISO string via `new Date(x).toISOString()` (activity-composer.tsx:119); `pyra_sales_follow_ups.status` enum = pending/completed/overdue/cancelled (NO `snoozed` — initial Q4-3 proposal had this wrong, corrected via pre-flight). Permission gate: `calendar.view` (later wired in Commit 5 — Commit 4 initially used `dashboard.view`, swapped in Commit 5). Scope: admin sees all; non-admin's `assigned_to` silently coerced to caller's username (URL-share-safe). Meetings (no `assigned_to`) scoped via `created_by`. Max window 62 days → 422 with Arabic message. LOCK 1 (CRITICAL): events sorted by `start ASC, source ASC, source_id ASC` (stable tiebreaker prevents UI flicker on refetch). LOCK 4 (CRITICAL): exactly 2 batched lookups per request (1× `pyra_sales_leads` for lead_name, 1× `pyra_users` for assignee display_name). 4 files (2 NEW + 2 MOD), +657 LOC. Smoke tests 47/47 (38 original + 9 TZ conversion). Reviewer LIGHT CONDITIONAL PASS — fixed CRITICAL TZ slippage: `new Date(x).toISOString()` was silently stripping the Dubai offset and emitting UTC `Z` strings, violating LOCK 3 JSDoc contract. Added `toDubaiIso(input)` helper (pure offset math — Dubai = UTC+4, no DST) applied to follow_up and meeting `start` fields. |
| 5 | `7c9f4e5` | **Calendar UI (4 views) + follow-up highlight handler.** Custom calendar built from date-fns primitives (Q4 lock: NOT react-big-calendar). NEW: `<CalendarEventPill>` (per-source icon + tone from `CALENDAR_EVENT_TONES`; full + compact variants; click → source-specific tab navigation per Q5-2), `<CalendarAgendaView>` (linear list grouped by Dubai-day with friendly date labels), `<CalendarDayView>` (24 hour-row grid + exports `bucketByHour` reused by week), `<CalendarWeekView>` (7-col × 24-hour grid with today-column highlight; all-day events in dedicated top row), `<CalendarMonthView>` (7×N cell grid with date number + 3 events per cell + `+N أكثر` overflow), `<CalendarToolbar>` (4-view tab switcher + prev/next/today + filter chips with `Check` icon). `<CalendarClient>` orchestrator — URL state for view+date+types via useSearchParams + router.replace shallow (LOCK 2); mobile default = agenda via matchMedia detection (Q5-1). Server page wrapper with `requirePermission('calendar.view')`. NEW RBAC permission `calendar.view` added to PERMISSIONS catalog + PERMISSION_CATALOG + BASE_EMPLOYEE so every internal user gets the calendar. NEW `dubaiDayKey()` helper in `lib/utils/format.ts` — Dubai-aware YYYY-MM-DD computation (same offset-math pattern as Commit 4's `toDubaiIso`). Follow-up highlight handler in `lead-sidebar.tsx` (LOCK 1) — mirrors Commit 1 pattern verbatim: `useSearchParams` reads `?followup={fu_id}` + `data-followup-id` on rendered card + querySelector(CSS.escape) + scrollIntoView + 2s ring-flash + cleanup (cancelAnimationFrame + clearTimeout + classList.remove). Graceful no-op when targeted follow-up isn't the sidebar's `limit:1` "next pending". Sidebar nav entry under الرئيسية group. Module guide entry. 13 files (8 NEW + 5 MOD), +1690/-7 LOC. Reviewer MEDIUM CONDITIONAL PASS — fixed HIGH 1 (TZ today-key bug: `.toISOString().slice(0,10)` returns UTC day → `dubaiDayKey` helper used in agenda + day views), HIGH 2 (RTL chevron convention: workspace standard from `components/layout/breadcrumb.tsx:106` is LTR-semantic icon + `rtl:rotate-180` utility — applied to prev/next buttons), MEDIUM (always-serialize view to URL — defends against viewport-rotation silent flip). |
| 6 | `c8d3ce7` | **Calendar widget on dashboard.** NEW `<MyCalendarWidget>` — compact 3-section preview (متأخرة / اليوم / هذا الأسبوع) from 7-day window. Permission gate via `usePermission('calendar.view')` returns null before any data fetch (Q6-1). This-week section caps at 5 events with `+N أخرى` link (Q6-2). Section headers as clickable `<Link>` elements with ArrowUpRight on hover (opacity-0 → opacity-100); destinations: overdue → `?view=agenda`, today → `?view=day`, week → `?view=week` (LOCK 1). Dashboard 2-col grid restructure (`grid-cols-1 lg:grid-cols-2 gap-4 items-start`) pairs MyWorkInbox + MyCalendarWidget on lg+; mobile stacks (LOCK 2). Refresh strategy: staleTime 60s + refetchOnWindowFocus + refetchOnMount 'always' — live-feel without polling (LOCK 3). `dubaiDayKey` used for all today-comparisons (LOCK regression prevention from Commit 5 HIGH 1). Reuses `CalendarEventPill` (DRY). Empty state: CalendarCheck icon + "لا أحداث قادمة" (no emoji per Phase 13). 2 files (1 NEW + 1 MOD), +302/-3 LOC. Reviewer LIGHT PASS — no fixes; 2 non-blocking notes (Section uses `<Link>` not `<button>` — accepted by LOCK 1 wording "buttons/anchors"; today memoized across midnight — v1.1 backlog). |
| 7 | (this commit) | **Closure docs** — CLAUDE.md "## Phase 15.1 — Locked Decisions" + this CRM-PROGRESS.md ✅ section + v1.1 backlog additions. |

### Architectural invariants locked

- **Mention highlight pattern (Commit 1 — re-used by Commit 5 follow-up highlight).** `useEffect` + `useSearchParams` reads ID query param + `querySelector('[data-X-id="' + CSS.escape(id) + '"]')` + `scrollIntoView({ behavior: 'smooth', block: 'center' })` + ring-2 flash via classList.add for 2 seconds + cleanup function that runs `cancelAnimationFrame` + `clearTimeout` + `classList.remove`. The pattern is now load-bearing for 2 different deep-link surfaces; any third instance MUST mirror this exact shape. data-attribute must use namespace prefix (`data-activity-id`, `data-followup-id`) so multiple highlight contexts on the same page don't collide.

- **Lead tasks as a parallel-to-attachments per-lead resource.** `pyra_lead_tasks` table mirrors the `pyra_lead_attachments` design exactly: `lead_id` FK with CASCADE + permission via `leads.update` + `canAccessLead` + cross-resource guard via double-eq + admin-OR-creator delete gate + activity dual-write. Future per-lead resources (notes-as-resource if extracted, tags when implemented) MUST follow this pattern — DO NOT invent new permission scopes for per-lead surfaces; reuse `leads.update`.

- **Calendar is a derived projection, not a new source of truth.** `/api/calendar/events` is READ-ONLY. Edits go through source-specific endpoints (`/api/crm/leads/[id]/tasks/*` for tasks, `/api/follow-ups/*` for follow-ups, `/api/crm/leads/[id]/activities` for meetings). Future "manual calendar event" feature (v1.1 backlog) requires a NEW table (`pyra_calendar_events`) — DO NOT shoehorn into the existing projection.

- **`dubaiDayKey()` is MANDATORY for any "today in Dubai" comparison.** `.toISOString().slice(0,10)` returns the UTC day, which differs from the Dubai day for the last 4 hours of every Dubai day. The helper applies pure offset math (Dubai = UTC+4, no DST). Any new component that derives a YYYY-MM-DD key for comparison against Dubai-offset ISO strings (the API's `start` format) MUST use `dubaiDayKey`. Phase 15.1 Commit 5 HIGH 1 surfaced this as a regression pattern that re-appeared in Commit 6 review — the Reviewer focus areas now explicitly check for `.toISOString().slice(0,10)` as a regression smell.

- **URL state persistence pattern for multi-view UIs.** Calendar (Commit 5) is the v1 reference implementation: `useSearchParams` reads view/date/types on mount + `router.replace(qs, { scroll: false })` writes shallow URL updates. Defaults are NOT serialized to URL (date=today, types=all → empty params); EXCEPT view (Commit 5 Reviewer MEDIUM fix — always serialize view to defend against viewport-rotation silent flip when `mobileDefault` differs from desktop default). Future multi-view UIs (e.g. Pipeline with view=kanban|table) MUST follow this pattern — back/forward + share-URL + refresh-preserves-state guarantees.

- **Section-header-as-link affordance pattern.** Dashboard widget (Commit 6) demonstrates: `<Link>` element + cursor-pointer + hover:bg-muted/50 + ArrowUpRight icon on hover (opacity transition) + ARIA label naming the destination context. Future widget headers with destination semantics (not just informational labels) MUST surface clickability with this affordance, NOT a bare div.

- **`logActivity()` action_type discipline (Phase 11.5 inheritance).** All Commit 2 + Commit 3 mutations on lead tasks emit `action_type = ${ENTITY_TYPES.LEAD}_${ACTIVITY_ACTIONS.UPDATE}` with the specific flavour in `metadata.source` (`'task_created'` / `'task_status_changed'` / `'task_updated'` / `'task_deleted'`). DO NOT introduce specific action_type strings like `'task_created'` directly — Phase 11.5 lock is in force.
...
### Phase 15.1 v1.1 items

- [ ] **Manual calendar events** (`pyra_calendar_events` table) — currently the calendar is a derived projection over 3 existing sources. v1.1 could add free-form events not tied to a lead (team meetings, deadlines, training sessions) via a NEW table. Architectural invariant locked: DO NOT shoehorn into the existing projection; new source = new table + new POST endpoint + extend the unified feed.
- [ ] **Drag-and-drop reschedule from the calendar** — currently clicks always open the source detail page; rescheduling happens there. v1.1 could add d&d on month/week/day views with optimistic update + rollback on PATCH failure. Q5 lock deferred this explicitly.
- [ ] **Email notifications for mentions** — currently `notifyMany()` only writes to `pyra_notifications` (in-app bell). v1.1 could fan out to email via the existing transactional mailer (when one is built — see Phase 9 Q-A1 decision deferring welcome email infrastructure).
- [ ] **Generic standalone tasks** (board-less, non-lead-attached) — `pyra_lead_tasks` is per-lead. v1 surfaces tasks ONLY through a lead OR through a project board. v1.1 could add a `pyra_personal_tasks` table for items like "follow up on payment from vendor X" that don't have a lead/board owner.
- [ ] **Calendar event recurrence** — currently each event is a single occurrence. v1.1 could add iCal-style `RRULE` (DAILY/WEEKLY/MONTHLY with COUNT/UNTIL) on lead tasks + follow-ups + meetings. Expansion happens at query time, NOT in the underlying tables.
- [ ] **Minute-precision positioning in week + day views** — currently events snap to hour rows (14:30 → 14:00 row, time shown in pill prefix). v1.1 could position blocks by minute offset (calendar-style overlap detection + side-by-side layout for conflicts). Adds ~150 LOC + new layout algorithm.
- [ ] **Side-by-side overlapping event layout** — currently up to 3 events per hour row + `+N أخرى` overflow link. v1.1 could implement Google-Calendar-style adjacent-column layout for overlapping events within the same time slot.
- [ ] **Edit Sheet for lead tasks (full field edit beyond inline)** — already shipped in Commit 3. Listed here as a placeholder for "Edit Sheet for follow-ups" + "Edit Sheet for meeting activities" if those surfaces grow similar density (currently both are managed inline at their source pages).
- [ ] **Bulk operations on lead tasks** — currently 1-at-a-time CRUD. v1.1 could add multi-select + bulk status change / bulk delete / bulk reassign. Low priority for 2-person team.
- [ ] **Midnight-staleness fix for MyCalendarWidget** — Phase 15.1 Commit 6 LOW finding: `today = useMemo(() => new Date(), [])` captures at mount. Tab left open across midnight keeps yesterday's `from`/`to` window. Fix: useEffect listener that re-mounts state at next midnight (Dubai TZ via `dubaiDayKey`). 2-person team edge case but worth doing.
- [ ] **Section header `<button>` semantic** (Commit 6 MEDIUM) — currently `<Link>` per LOCK 1 "buttons/anchors" allowance. v1.1 could switch to `<button>` + programmatic `router.push()` for tighter button-vs-link semantics. Functional behavior identical.
- [ ] **Lead-sidebar follow-up rendering** — currently shows only `nextFollowUp` (`limit:1`). Highlight handler is graceful no-op when the targeted follow-up isn't the current "next". v1.1 could render top N follow-ups so any calendar-click target is always findable.
- [ ] **Custom waveform / availability picker for meeting metadata** — currently `metadata.location` is free-text. v1.1 could add timezone-aware time picker + meeting-link normalization (Zoom URL → `Zoom` label etc.).
...
- [ ] **pg_dump availability check** at `pnpm dev` startup or first-migration time — currently the dev hits the error only when they try `pnpm db:backup`. A pre-flight check at install time would surface the missing-tool sooner. *Partially mitigated 2026-07-03:* pg_dump 17.5 installed portably on Abdou's machine (`%LOCALAPPDATA%\Programs\pgsql\bin`, on user PATH) and `db-backup.sh` now auto-probes that location + `C:\Program Files\PostgreSQL\<ver>\bin` before erroring (see `docs/MIGRATIONS.md` §14). **Remaining blocker for a real backup: `SUPABASE_DB_URL` is not set in `.env.local`** — needs the direct Postgres connection string from Coolify (and the Postgres port must be reachable from the dev machine; 5432/6543 on the Kong host are closed).
